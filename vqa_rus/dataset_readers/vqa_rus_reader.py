...
@DatasetReader.register("vqa_rusv2")
class VQArusReader(VisionReader):
    """
    Parameters
    ----------
    image_dir: `str`
    ...
    def __init__(
        ...
        *,
        ...
        if self.produce_featurized_images:
            ...
            self.images = {
                id_from_filename(name): full_path
                for name, full_path in self.images.items()
            }
    ...
    def _read(self, splits_or_list_of_splits: Union[str, List[str]]):
        ...
        questions = []
        with open(
            cached_path(split.questions, extract_archive=True), encoding="utf-8"
        ) as f:
        ...
        if self.produce_featurized_images:
            ...
            try:
                image_paths = [
                    self.images[int(question_dict["image_id"])]
        ...
        for question_dict, processed_image in zip(question_dicts, processed_images):
            ...
            instance = self.text_to_instance(
                question_dict["question"], processed_image, answers
            )
            ...
            if attempted_instances_count % 2000 == 0:
                failed_instances_fraction = (
                    failed_instances_count / attempted_instances_count
                )
    ...
    def _get_answers_by_question_id(self, split):
        ...
        if split.annotations is not None:
            ...
            with LocalCacheResource(
                split.annotations + "-cache", annotations_path
            ) as cache:
                if cache.cached():
                    logger.info(
                    ...
                    for a in annotations["annotations"]:
                        ...
                        if self.multiple_answers_per_question:
                            for answer in (
                                answer_dict["answer"] for answer_dict in a["answers"]
                            ):
                                answer_counts[utils.preprocess_answer(answer)] += 1
                        else:
                            answer_counts[
                                utils.preprocess_answer(a["multiple_choice_answer"])
                            ] = 1
